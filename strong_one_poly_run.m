...
c(7) = 144;     % Oi -> Ai
c(8) = 1000;     % Oi -> Oi(paused) for i = 16 to 50
c(9) = 0.002;   % Oi(paused) -> Oi for i = 16 to 50
c(10) = 0.0032; % An -> TC
c(11) = 0.0032; % TC -> RNA + Un

% Simulation parameters
RNAsynth_target = int32(100000);      % Number of RNAs to synthesize
tmax = 12000;
noutput = 24000;
...
for i=1:RNAsynth_target
    [position(i,:)] = strong_one_poly_simplified(n_nuc,statenames,c,t);
end
...
% end
% Save data.
save strong_one_poly_simplified-1000kp.mat -v7.3
 dy=diff(pterminated)./diff(t);
% plot(t(2:end),dy);
 w=400;
  %course_grained_prob=zeros((t-1)/w,'int32');
